...
for(const q of parts){x.globalAlpha=q.a;x.fillStyle="#fff";x.beginPath();x.arc(q.x,q.y,q.s,0,Math.PI*2);x.fill()}x.globalAlpha=1;if(bird){x.save();x.translate(bird.x,bird.y);x.rotate(bird.rot);x.shadowColor="rgba(0,0,0,.18)";x.shadowBlur=6;x.shadowOffsetY=3;x.fillStyle="#ffd447";x.beginPath();x.arc(0,0,19,0,Math.PI*2);x.fill();x.shadowColor="transparent";x.fillStyle="#f2a632";x.beginPath();x.ellipse(-8,7,13,7,0,0,Math.PI*2);x.fill();x.fillStyle="#fff";x.beginPath();x.arc(8,-7,7,0,Math.PI*2);x.fill();x.fillStyle="#18202b";x.beginPath();x.arc(10,-7,2.5,0,Math.PI*2);x.fill();x.fillStyle="#f47735";x.beginPath();x.moveTo(16,-3);x.lineTo(30,2);x.lineTo(16,7);x.closePath();x.fill();x.restore()}x.restore();if(state==="play"||state==="pause"){drawPause();drawScore();drawBest()}if(state==="title")titlePanel();if(state==="pause")overlayPanel("PAUSED","TAP TO RESUME");if(state==="over")gameOverPanel();shake*=.82}
...
addEventListener("pointerdown",e=>{
  const px=e.clientX/innerWidth*W, py=e.clientY/innerHeight*H;
  if(state==="play"&&px>=8&&px<=78&&py>=45&&py<=105){state="pause";return}
  if(state==="pause"){
    if(px>=92&&px<=308&&py>=445&&py<=489){state="title";reset();return}
    state="play";return;
  }
  if(state==="over"){
    if(px>=92&&px<=308&&py>=445&&py<=489){state="title";reset();return}
